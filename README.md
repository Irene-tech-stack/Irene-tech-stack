# Hi 👋, I'm Irene Wanjugu Weru

### Customer Experience Leader | Operations Manager | Data Analytics | Machine Learning Enthusiast | AI Operations

I'm passionate about improving customer experiences, optimizing business operations, and using data to drive better decision-making. With over 8 years of experience in customer experience, operations management, and business process improvement, I enjoy solving complex business challenges through analytics, process optimization, and technology.

---

## 👩‍💼 About Me

- 💼 Customer Experience & Operations Professional
- 📊 Data Analytics & Business Intelligence
- 🤖 Machine Learning Enthusiast
- 📈 Customer Retention & Churn Analysis
- 🚀 Process Improvement & Operational Excellence
- 🌍 Based in Nairobi, Kenya

---

## 🛠️ Skills

### Data & Analytics
- Microsoft Excel (Advanced)
- Google Sheets
- SQL
- Power BI
- Data Visualization
- KPI Reporting

### Machine Learning
- Customer Churn Prediction
- Predictive Analytics
- Data Cleaning
- Model Evaluation

### Operations & Leadership
- Customer Experience
- Customer Success
- Stakeholder Management
- Process Improvement
- SOP Development
- Performance Management
- Cross-functional Collaboration

### Tools
- Git & GitHub
- Microsoft Office
- CRM Systems
-SQL workbench
-Power BI
-Splynx
-Zabbix


---

## 🚀 Featured Projects

### 📌 Customer Churn Prediction Model
Developed a machine learning model to identify customers at risk of churning, enabling proactive customer retention strategies through predictive analytics.

### 📌 Customer Service Analytics Dashboard
Built dashboards to monitor operational KPIs including call volumes, repeat contacts, ticket conversion, SLA compliance, and team productivity.

### 📌 Customer Segmentation
Applied data analysis techniques to segment customers based on usage and behavior, helping improve engagement and customer retention strategies.

### 📌 Operations Reporting
Designed reporting templates, performance dashboards, and operational scorecards to improve decision-making and team efficiency.

---

## 🌱 Currently Learning

- Artificial Intelligence Operations (AI Ops)
- Large Language Models (LLMs)
- Advanced SQL

---

## 🤝 Let's Connect

📧 **Email:** ireneweru9803@gmail.com

💼 **LinkedIn:** www.linkedin.com/in/irene-weru-824b3014b

---

> *"Transforming data into insights, operations into excellence, and customer experiences into lasting value."*
